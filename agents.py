...
def analyzer_agent(query, llm):
    return llm.invoke(f"""
    You are the EduCortex Analyzer Agent.
...
def structure_and_polish_agent(answer, llm, wants_long=False):
    ...
    return llm.invoke(f"""
    You are the EduCortex Structure & Polish Agent.
...
def summarizer_agent(answer, llm):
    return llm.invoke(f"""
    You are the EduCortex Summarizer Agent.
...
def feedback_agent(user_answers, questions, context, llm):
    return llm.invoke(f"""
    You are the EduCortex Evaluator Agent.
...
"""
    ...
    return llm.invoke(f"""
    You are the EduCortex Question Generator Agent.
