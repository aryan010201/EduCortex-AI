...
from backend import process_query, EduCortexMemory
...
def get_current_session():
    ...
    return {"title": "New Chat", "chat": [], "memory": EduCortexMemory()}
...
# ============================================
prompt = st.chat_input("Message EduCortex AI...", accept_file=True, file_type=["pdf", "docx", "txt"])
...
with st.sidebar:
    ...
    <div style="padding:14px 4px 10px 4px;">
        <div style="font-size:1.05rem; font-weight:800; color:#a5b4fc;">EduCortex AI</div>
    ...
    # Memory
    memory = current_session.get("memory", EduCortexMemory())
...
<div style="text-align:center; margin-bottom:1.5rem; padding-top:4px;">
    ...
    <h1 style="font-size:3.2rem; font-weight:900; margin:8px 0 5px 0; color:#111827; letter-spacing:-1px;">EduCortex AI</h1>
...
if not current_chat:
    ...
    <div style="background:white; border:1px solid #e5e7eb; border-radius:14px; padding:24px; margin-bottom:1.5rem;">
        <h3 style="text-align:center; color:#374151; margin-bottom:18px; font-size:1rem;">How to use EduCortex AI</h3>
...
if prompt and prompt.text:
    ...
    with st.spinner("EduCortex is thinking..."):
    ...
    if result.get("answer"):
        final_html += f"""<div class="agent-output-box" style="border-left-color:#059669;"><div class="agent-output-label" style="color:#059669;">EduCortex AI</div><div style="white-space:pre-line; font-size:0.9rem;">{result['answer']}</div></div>"""
